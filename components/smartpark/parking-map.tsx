...
const rowGroups = [['A', 'B'], ['C', 'D'], ['E', 'F']] as const
...
export function ParkingMap({ mode = 'readonly', compact = false, variant = 'default', floorId: initialFloorId = 'B1', selectableSlotIds = [], selectedSlotId, onSelectSlot }: ParkingMapProps) {
  ...
  return <div className={compact ? 'min-w-0' : 'mx-auto max-w-[1440px]'}>
    ...
    <section className={`floorplan-card ${variant === 'chat' ? 'floorplan-card-chat' : ''}`}><div className="floorplan-scroll"><div className="floorplan-shell">
      ...
      <div className="floorplan-main"><div className="floorplan-row-group">{rowGroups[0].map((row) => <div key={row} className="floorplan-row">{floor.zones.find((zone) => zone.id === row)?.slots.map((slot) => <SlotButton key={slot.id} slot={slot} mode={mode} selectableSlotIds={selectableSlotIds} selectedSlotId={selectedSlotId} selected={selected} onClick={() => selectSlot(slot)} />)}</div>)}<div className="floorplan-divider" /></div><div className="floorplan-middle"><div className="floorplan-row-split"><div className="floorplan-row">{floor.zones.find((zone) => zone.id === 'C')?.slots.map((slot) => <SlotButton key={slot.id} slot={slot} mode={mode} selectableSlotIds={selectableSlotIds} selectedSlotId={selectedSlotId} selected={selected} onClick={() => selectSlot(slot)} />)}</div><LiftBlock central /><div className="floorplan-row">{floor.zones.find((zone) => zone.id === 'D')?.slots.map((slot) => <SlotButton key={slot.id} slot={slot} mode={mode} selectableSlotIds={selectableSlotIds} selectedSlotId={selectedSlotId} selected={selected} onClick={() => selectSlot(slot)} />)}</div></div><div className="floorplan-aisle"><ArrowRight /><ArrowRight /></div></div><div className="floorplan-divider" />{rowGroups[2].map((row) => <div key={row} className="floorplan-row">{floor.zones.find((zone) => zone.id === row)?.slots.map((slot) => <SlotButton key={slot.id} slot={slot} mode={mode} selectableSlotIds={selectableSlotIds} selectedSlotId={selectedSlotId} selected={selected} onClick={() => selectSlot(slot)} />)}</div>)}</div>
