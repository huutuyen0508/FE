...
const statusLabel: Record<ParkingSlotStatus, string> = { available: 'Available', occupied: 'Occupied', reserved: 'Reserved', special: 'Other / Special', out_of_service: 'Out of service' }
...
export function ParkingMap({ mode = 'readonly', compact = false, variant = 'default', floorId: initialFloorId = 'B1', selectableSlotIds = [], selectedSlotId, onSelectSlot }: ParkingMapProps) {
  ...
  return <div className={compact ? 'min-w-0' : 'mx-auto max-w-[1440px]'}>
    ...
    {variant !== 'chat' && <div className="floorplan-header"><div><h1>Ground Floor Plan</h1><p>Live parking availability and zoning.</p></div><div className="floorplan-legend">{statusOptions.filter(({ value }) => value !== 'out_of_service').map(({ value, label }) => <span key={value}><i className={`legend-swatch legend-swatch-${value}`} />{value === 'special' ? 'Other / Special' : label}</span>)}<span><i className="legend-swatch legend-swatch-mine" />Your Car</span></div></div>}
